...
func NewGoogleSheetsProvider(ctx context.Context, gp *GoogleProvider) (*GoogleSheetsProvider, error) {
	srv, err := sheets.NewService(ctx, option.WithHTTPClient(gp.client))
	if err != nil {
...
func (p *GoogleSheetsProvider) Parse(ctx context.Context, excelFile *models.ExcelFile) (*models.ExcelFile, error) {
...
func (p *GoogleSheetsProvider) parseSheet(rows [][]interface{}, mySheet *models.Sheet) {
	mySheet.Rows = make([]*models.Row, 0)

	for _, row := range rows {
		myRow := &models.Row{}
		myRow.Cells = make([]*models.Cell, 0)
